...
from movies.models import Movie,MoviesCollection,ProductionCountry,ProductionCompany,SpokenLanguage,MovieGenre,Keyword,VotedMovieGenre,VotedKeyword,SimilarMovie,RecommendedMovie
from relations.models import MovieBookSimilarRelation,MovieBookRecommendationRelation
...
class SimilarMovieSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = SimilarMovie
        fields = ('id','movie','similar_movie','similarity_index')
...
class RecommendedMovieSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ('id','movie','recommended_movie','recommendation_index')

class SimilarBookSerializer(serializers.ModelSerializer):
    book = BookCompactSerializer()
    class Meta:
        model = MovieBookSimilarRelation
        fields = ('id','book','similarity_index','movie')

class RecommendedBookSerializer(serializers.ModelSerializer):
    book = BookCompactSerializer()
    class Meta:
        model = MovieBookRecommendationRelation
        fields = ('id','book','recommendation_index','movie')
...
class MovieSerializer(serializers.ModelSerializer):
    ...
    similar_books_info = SimilarBookSerializer(many=True,allow_null=True,required=False)
    recommended_books_info = RecommendedBookSerializer(many=True,allow_null=True,required=False)
    class Meta:
        model = Movie
        fields = ('id','tmdb_id','original_title','title','poster_path','backdrop_path','adult','budget','homepage','original_language','overview','popularity','release_date','revenue','runtime','status','tagline','video','tmdb_average_rating','tmdb_vote_count','average_rating','votes_count','belongs_to_collection','production_countries','production_companies','spoken_languages','genres_related','keywords','genres_info','keywords_info','similar_movies_info','recommended_movies_info','similar_books_info','recommended_books_info')
